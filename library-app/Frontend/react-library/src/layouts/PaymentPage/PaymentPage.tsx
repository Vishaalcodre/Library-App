...
export const PaymentPage = () => {
    ...
    useEffect(() => {
        const fetchFees = async () => {
            if(authState && authState.isAuthenticated){
                ...
                const requenseOptions = {
                    method: "GET",
                    headers: {"Content-Type": "application/json"}
                };
                const paymentResponse = await fetch(url, requenseOptions);
    ...
    async function checkout() {
        ...
        let paymentInfo = new PaymentInfoRequest(Math.round(fees * 10), "IND", authState?.accessToken?.claims.sub);
    ...
    return(
        <div className="container">
            {fees > 0 && <div className="card mt-3">
                <h5 className="card-header">Fees pending: 
                ...
                <div className="card-body">
                    ...
                    <button disabled={submitDisabled} type="button" className="btn btn-md main-color 
                    text-white mt-3" onClick={checkout}>
                        Pay fees
                    </button>
                </div>
                </div>
